...
const Species = () => {
  ...
  useEffect(() => {
    const fetchData = async (apiKey) => {
      const response = await fetch(`${onfarmAPI}/raw?table=cc_mixture&complete_count=code`, {
        headers: {
          'x-api-key': apiKey,
        },
      });
      ...
      {fetching ? (
        ...
        <Fragment>
          ...
          <Grid item container xs={12}>
            <SpeciesTable data={farmValues} farmYears={farmYears} affiliations={affiliations} />
          </Grid>
        </Fragment>
      )}
    </Grid>
