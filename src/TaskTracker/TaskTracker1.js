...
import TaskTrackerCard1 from "./TaskTrackerCard1";
...
const TaskTracker1 = () => {
  ...
  useEffect(() => {
    const fetchData = async (apiKey) => {
        const response = await fetch(`${onfarmAPI}/raw?table=decomp_bag`, {
            headers: {
          "x-api-key": apiKey,
        },
      });
  ...
  useEffect(() => {
    const fetchData = async (apiKey) => {
        const response = await fetch(`${onfarmAPI}/raw?table=decomp_biomass_dry&affiliation=${activeAffiliation()}&year=${activeFarmYear()}`, {
            headers: {
          "x-api-key": apiKey,
        },
      });
