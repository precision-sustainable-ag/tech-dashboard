...
import { Typography, TableRow, TableCell, Table, TableBody, TableContainer, TableHead } from "@material-ui/core";
...
const ActualFarmDates = ({rowData, fetchFromApi}) => {
    ...
    const parseDates = async (rowData) => {
        fetchFromApi(`${dryBiomass_url}&code=${rowData.code}`, state.userInfo.apikey)
            .then(res => {
                // console.log(res)
                ...
                for(const r of res){
                    ...
                    if(r.subplot === 1 && r.subsample === "A")
                        dates.s1sa.push(r);
                    else if(r.subplot === 1 && r.subsample === "B")
                        dates.s1sb.push(r);
                    else if(r.subplot === 2 && r.subsample === "A")
                        dates.s2sa.push(r);
                    else if(r.subplot === 2 && r.subsample === "B")
                        dates.s2sb.push(r);
                }
        ...
        return (
            <TableRow>
                ...
                {data.map((date, index) => {
                    return (
                        <TableCell key={index}>
                            <Typography variant="subtitle2">{date.recovery_date ? new Date(date.recovery_date).toLocaleDateString('en-US', {timeZone: 'UTC'}) : ""}</Typography>
                        </TableCell>
                    )
                })}
            </TableRow>
            ...
            <Table size="medium" stickyHeader>
                <TableHead>
                    <TableRow>
                        ...
                        <TableCell><Typography variant="subtitle1">T4 Actual</Typography></TableCell>
